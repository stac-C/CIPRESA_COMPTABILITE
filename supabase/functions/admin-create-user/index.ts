...
Deno.serve(async (request) => {
  ...
  const { data: assignedRoles, error: roleError } = await adminClient.from("user_roles").select("role:roles(code)").eq("user_id", user.id);
  if (roleError) return response({ error: `Vérification des droits impossible: ${roleError.message}` }, 500);
  if (!(assignedRoles || []).some(({ role }) => role?.code === "ADMIN")) return response({ error: "Droits administrateur requis." }, 403);

  let payload;
  try {
    payload = await request.json();
  } catch {
    return response({ error: "Le formulaire envoyé est invalide." }, 400);
  }
