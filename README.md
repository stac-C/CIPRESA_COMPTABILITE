...
### Activer tous les sous-onglets comptables

Apres le schema principal, executez [20260907_accounting_module.sql](supabase/migrations/20260907_accounting_module.sql) dans l'editeur SQL Supabase. Cette migration ajoute les tables `rapprochements_bancaires`, `declarations_fiscales`, `immobilisations` et `clotures_comptables`, leurs policies RLS pour `COMPTABLE`, les permissions correspondantes ainsi que les vues du grand livre et du compte de resultat.

Le role `COMPTABLE` conserve les permissions `COMPTA_READ`, `COMPTA_CREATE`, `COMPTA_VALIDATE`, `BILAN_READ`, `BILAN_GENERATE`, `RAPPORT_READ` et `RAPPORT_CREATE`. Les sous-onglets chargent ensuite leurs lignes depuis Supabase et affichent une erreur explicite si la migration n'a pas encore ete executee.
