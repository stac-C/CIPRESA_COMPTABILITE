...
import { Bell, BellOff, Camera, Check, ImagePlus, LogOut, LockKeyhole, MonitorSmartphone, Moon, Save, ShieldCheck, Sun, Trash2, X } from "lucide-react";
...
function ConfirmDialog({ confirmation, onCancel }) {
  ...
  return <div className="modal-backdrop" role="presentation"><div className="success-dialog confirm-dialog" role="dialog" aria-modal="true" aria-label="Confirmer l’action"><h2>{confirmation.title}</h2><p>{confirmation.description}</p><div className="form-actions"><button className="btn-secondary" type="button" onClick={onCancel}>Annuler</button><button className="primary-button" type="button" onClick={confirmation.action}>Confirmer</button></div></div></div>;
}
