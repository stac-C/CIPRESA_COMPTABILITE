...
function AdminCreateUser({ onMessage }) {
  ...
  async function createUser(event) {
    ...
    setSaving(false);
    if (error || data?.error) {
      const text = data?.error || (error?.message?.includes("Function not found") ? "La fonction admin-create-user n’est pas encore déployée dans Supabase." : error?.message || "La création du compte a échoué.");
