...
function GeneralLedgerView(){const [account,setAccount]=useState('411100');const rows=journalEntries.filter(r=>r.account===account);let balance=0;return <SectionCard title="Grand livre — comptes mouvementés" description="Vue chronologique par compte, avec solde progressif."><div className="toolbar"><select value={account} onChange={e=>setAccount(e.target.value)}>{['411100','401101','701100','601101'].map(a=><option key={a}>{a}</option>)}</select><button className="outline-button" type="button"><Download size={14}/> Exporter</button></div><div className="ledger-summary"><div><span>Compte sélectionné</span><strong>{account}</strong></div><div><span>Solde d’ouverture</span><strong>0 XAF</strong></div><div><span>Solde courant</span><strong>{money(rows.reduce((a,r)=>a+r.debit-r.credit,0))}</strong></div></div><div className="table-scroll"><table className="data-table accounting-table"><thead><tr><th>Date</th><th>Pièce</th><th>Libellé</th><th className="num">Débit</th><th className="num">Crédit</th><th className="num">Solde</th></tr></thead><tbody>{rows.map((r,i)=>{balance += r.debit-r.credit;return <tr key={i}><td>{r.date}</td><td className="code-cell">{r.number}</td><td>{r.label}</td><td className="num">{money(r.debit)}</td><td className="num">{money(r.credit)}</td><td className="num">{money(balance)}</td></tr>})}</tbody></table></div></SectionCard>}
...
function ReconciliationView(){const [items,setItems]=useState([['BQ-2025-0312','Virement Ministère Agriculture','15 000 000','Rapproché'],['CAI-O-1022','Encaissement comptoir Otele','850 000','À contrôler'],['MM-MTN-2205','Commission MTN','25 000','Écart']]);return <SectionCard title="Rapprochement bancaire" description="Contrôle entre relevés et écritures internes." actions={<button className="outline-button" type="button" onClick={()=>setItems(current=>current.map(x=>x[3]==='À contrôler'?[x[0],x[1],x[2],'Rapproché']:x))}><RefreshCw size={14}/> Rapprocher</button>}><div className="table-scroll"><table className="data-table accounting-table"><thead><tr><th>Référence</th><th>Libellé</th><th className="num">Montant</th><th>État</th></tr></thead><tbody>{items.map((r,i)=><tr key={i}><td className="code-cell">{r[0]}</td><td>{r[1]}</td><td className="num">{r[2]} XAF</td><td><span className="badge">{r[3]}</span></td></tr>)}</tbody></table></div></SectionCard>}
...
function ClosuresView(){const [checks,setChecks]=useState({balance:false,bank:false,taxes:false});const ready=Object.values(checks).every(Boolean);return <SectionCard title="Clôtures comptables" description="Sécurisez les contrôles avant de fermer une période."><div className="toggle-grid">{[['balance','Balance contrôlée'],['bank','Rapprochement bancaire contrôlé'],['taxes','Taxes contrôlées']].map(([id,label])=><div className="toggle-option" key={id}><button className={`toggle ${checks[id]?'on':''}`} type="button" onClick={()=>setChecks(c=>({...c,[id]:!c[id]}))}><span/></button><div><strong>{label}</strong><small>{checks[id]?'Validé':'À contrôler'}</small></div></div>)}</div><div className="closure-list"><ClosureRow period="Septembre 2025" status="Clôturée" done/><ClosureRow period="Octobre 2025" status={ready?'Prêt à clôturer':'Contrôles en cours'}/><ClosureRow period="Exercice 2025" status="Pré-clôture"/></div></SectionCard>}
function ClosureRow({period,status,done}){return <div className="closure-row"><div className="closure-icon">{done?<Check size={16}/>:<LockKeyhole size={14}/>}</div><div><strong>{period}</strong><span>{status}</span></div><button className="outline-button" type="button">{done?'Consulter':'Préparer'}</button></div>}
