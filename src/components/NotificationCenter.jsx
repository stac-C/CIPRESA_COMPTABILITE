...
export default function NotificationCenter() {
  ...
  const { notifications, unreadCount, markAsRead, enableBrowserNotifications } = useRealtimeNotifications(session?.user?.id);
  ...
  return <div className="platform-tools">
    <div className="platform-tool-group">
      ...
      {open === "notifications" && <section className="tool-popover notifications-popover" aria-label="Centre de notifications"><div className="tool-popover-header"><div><strong>Notifications</strong><small>{unreadCount} non lue{unreadCount > 1 ? "s" : ""}</small></div><button className="link-button" type="button" onClick={enableBrowserNotifications}>Activer sur cet appareil</button></div>{notifications.length === 0 ? <p className="empty">Aucune notification pour le moment.</p> : <div className="notification-list">{notifications.map((notification) => <button className={notification.lu ? "notification-item is-read" : "notification-item"} type="button" key={notification.id} onClick={() => markAsRead(notification.id)}><strong>{notification.titre}</strong><span>{notification.message}</span><small>{new Date(notification.created_at).toLocaleString("fr-FR")}</small></button>)}</div>}</section>}
    </div>
    <div className="platform-tool-group">
