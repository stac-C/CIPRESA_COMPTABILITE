...
export default function ResourceWorkspace({ resource, can }) {
  ...
  async function save(event) {
    ...
    const request = editingId
      ? supabase.from(resource.table).update(form).eq("id", editingId)
      : supabase.from(resource.table).insert(form);
  ...
  return (
    <div className="resource-workspace">
      ...
      <section className="content-panel">
        ...
        {loading ? <p className="empty">Chargement...</p> : rows.length === 0 ? <p className="empty">Aucune donnée accessible pour ce rôle.</p> : <div className="table-scroll"><table className="data-table"><thead><tr>{resource.columns.filter((column) => column !== "id").map((column) => <th key={column}>{titleForColumn(column)}</th>)}{(canUpdate || canDelete) && <th>Opérations</th>}</tr></thead><tbody>{rows.map((row) => <tr key={row.id}>{resource.columns.filter((column) => column !== "id").map((column) => <td key={column}>{displayValue(row[column])}</td>)}{(canUpdate || canDelete) && <td className="table-actions">{canUpdate && <button className="link-button" type="button" onClick={() => startEdit(row)}>Modifier</button>}{canDelete && <button className="link-button danger" type="button" onClick={() => remove(row)}>Supprimer</button>}</td>}</tr>)}</tbody></table></div>}
      </section>
    </div>
  );
}
