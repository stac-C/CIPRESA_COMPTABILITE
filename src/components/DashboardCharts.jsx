...
export default function DashboardCharts({ invoices = [] }) {
  const byMonth = invoices.reduce((result, invoice) => {
    const month = invoice.date_facture?.slice(0, 7) || "Inconnu";
    result[month] = (result[month] || 0) + Number(invoice.montant_ttc || 0);
    return result;
  }, {});
  ...
  return <div className="overview-grid">
    <section className="content-panel chart-panel"><div className="section-heading"><div><p className="section-kicker">Factures</p><h2>Chiffre d’affaires sur six mois</h2></div></div><div className="chart-box"><ResponsiveContainer width="100%" height={240}><LineChart data={evolution}><XAxis dataKey="month" /><YAxis /><Tooltip formatter={(value) => `${Number(value).toLocaleString("fr-FR")} XAF`} /><Line type="monotone" dataKey="total" stroke="#0e7490" strokeWidth={3} dot={{ r: 4 }} /></LineChart></ResponsiveContainer></div></section>
