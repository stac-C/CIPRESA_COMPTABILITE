...
function DashboardOverview({ loading, error, metricErrors, stats, factures, ventes }) {
  const overdueInvoices = factures.filter((invoice) => invoice.statut === "EN_RETARD");
  const pendingInvoices = factures.filter((invoice) => ["EMISE", "PARTIELLEMENT_PAYEE"].includes(invoice.statut));
  const paidInvoices = factures.filter((invoice) => invoice.statut === "PAYEE");
  const collectionRate = stats.chiffreAffaires > 0 ? Math.max(0, Math.min(100, ((stats.chiffreAffaires - stats.resteAPayer) / stats.chiffreAffaires) * 100)) : 0;
  const recentSales = [...ventes].sort((left, right) => String(right.date_vente || "").localeCompare(String(left.date_vente || ""))).slice(0, 4);
  const formatMoney = (value) => `${Number(value || 0).toLocaleString("fr-FR")} XAF`;

  return <>{error && <p className="message error">Erreur de chargement : {error}</p>}{!error && metricErrors.length > 0 && <p className="message warning">Certaines métriques ne sont pas disponibles avec les policies RLS actuelles. Les indicateurs affichés restent limités aux données autorisées.</p>}{loading ? <p className="empty">Chargement des données…</p> : <>
    <section className="executive-summary">
      <div><p className="section-kicker">Pilotage opérationnel</p><h2>Vue d’ensemble de votre activité</h2><p>Suivez les flux commerciaux, la facturation et les encaissements depuis les données accessibles à votre rôle.</p></div>
      <div className="summary-period"><span>Période suivie</span><strong>Six derniers mois</strong><small>{stats.factures.toLocaleString("fr-FR")} facture{stats.factures > 1 ? "s" : ""} · {stats.ventes.toLocaleString("fr-FR")} vente{stats.ventes > 1 ? "s" : ""}</small></div>
    </section>
    <section className="stats-grid executive-kpis"><StatCard label="Chiffre d’affaires" value={formatMoney(stats.chiffreAffaires)} hint="Ventes visibles" /><StatCard label="Créances clients" value={formatMoney(stats.resteAPayer)} hint="Montant restant à recouvrer" /><StatCard label="Taux d’encaissement" value={`${collectionRate.toFixed(0)} %`} hint={`${paidInvoices.length} facture${paidInvoices.length > 1 ? "s" : ""} payée${paidInvoices.length > 1 ? "s" : "e"}`} /><StatCard label="Activité commerciale" value={stats.ventes.toLocaleString("fr-FR")} hint={`${stats.clients.toLocaleString("fr-FR")} clients accessibles`} /><StatCard label="Factures en attente" value={pendingInvoices.length.toLocaleString("fr-FR")} hint="Émises ou partiellement payées" /><StatCard label="Produits en catalogue" value={stats.produits.toLocaleString("fr-FR")} hint="Référentiel disponible" /></section>
    <section className="dashboard-insights">
      <article className="content-panel insight-panel"><div className="section-heading"><div><p className="section-kicker">À surveiller</p><h2>Priorités de gestion</h2></div><span className="insight-score">{overdueInvoices.length > 0 ? "Action requise" : "Situation maîtrisée"}</span></div><div className="insight-list"><div className={overdueInvoices.length ? "insight-item danger" : "insight-item success"}><span className="insight-indicator"/><div><strong>{overdueInvoices.length ? `${overdueInvoices.length} facture${overdueInvoices.length > 1 ? "s" : ""} en retard` : "Aucune facture en retard"}</strong><small>{overdueInvoices.length ? `Créances concernées : ${formatMoney(overdueInvoices.reduce((sum, invoice) => sum + Number(invoice.reste_a_payer || 0), 0))}` : "Le portefeuille facturé ne présente pas d’échéance dépassée."}</small></div></div><div className="insight-item neutral"><span className="insight-indicator"/><div><strong>{pendingInvoices.length} facture{pendingInvoices.length > 1 ? "s" : ""} à suivre</strong><small>Contrôlez les échéances et les paiements partiels.</small></div></div></div></article>
      <article className="content-panel insight-panel"><div className="section-heading"><div><p className="section-kicker">Derniers mouvements</p><h2>Activité commerciale</h2></div></div>{recentSales.length === 0 ? <p className="empty">Aucune vente récente accessible.</p> : <div className="recent-activity-list">{recentSales.map((sale, index) => <div className="recent-activity" key={sale.id || sale.numero || index}><span className="activity-date">{sale.date_vente || "Date non renseignée"}</span><div><strong>{sale.numero || "Vente sans numéro"}</strong><small>{sale.statut || "Statut non renseigné"}</small></div><b>{formatMoney(sale.total)}</b></div>)}</div>}</article>
    </section>
    <DashboardCharts invoices={factures} sales={ventes} />
    <FacturesDataTable factures={factures} />
  </>}</>;
}
