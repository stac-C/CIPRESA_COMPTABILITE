...
export default function useRealtimeNotifications(userId, enabled = true) {
  ...
  async function deleteNotification(id) {
    ...
    if (!error) setNotifications((current) => current.filter((item) => item.id !== id));
    return { error };
  }
  ...
  return { notifications, unreadCount: notifications.filter(({ lu }) => !lu).length, markAsRead, deleteNotification, enableBrowserNotifications };
}
